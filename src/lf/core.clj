...
(defn interpretar [cod regs-de-act cont-prg pila mapa-regs]
  (let [fetched (cod cont-prg),
       ...
       (case opcode
          ...
          CHR (let [res (aplicar-operador-diadico get pila)]
               (if (nil? res) res (recur cod regs-de-act (inc cont-prg) res mapa-regs)))
...
(defn listar [tokens]
  (print (listar-str "" tokens 0 true))
)
...
(defn agregar-ptocoma 
  ([tokens] (agregar-ptocoma '() tokens))
  ([prev tokens] 
    (let [token (first tokens) next (second tokens) rest (rest tokens)]
      (cond
        (empty? tokens) (reverse prev)
        (and (= token (symbol "}")) (deberia-agregar-ptocoma? next)) (recur (conj prev token (symbol ";")) rest)
        :else (recur (conj prev token) rest)
      )
    )
  )
)
...
; false
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defn ya-declarado-localmente? [id [scopes declaraciones]] ()
  (some (comp (partial = id) first) (drop (last scopes) declaraciones))
)
...
; nil
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defn buscar-tipo-de-retorno [ambiente ref] ()
  (let [buscado (first (filter (comp (partial = ref) last) (last (contexto ambiente))))]
    (if (nil? buscado)
      nil
      (-> buscado (get ,,, 1) (get ,,, 1) (last ,,,)) ; buscado es por ejemplo: ['main ['fn [() ()]] 8]
    )
  )
)
...
(defn generar-ref [ambiente] ()
  (let 
    [
      var-buscada (last (simb-ya-parseados ambiente))
      buscado (last (filter (comp (partial = var-buscada) first) (last (contexto ambiente))))
    ]
    (generar ambiente 'PUSHADDR (last buscado))
  )
)
...
(defn pasar-a-int [n]
  (cond 
    (number? n) (int n)
    (string? n) (try (Integer/parseInt n) (catch Exception _ n))
    :else n
  )
)
...
; [10]
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defn pasar-a-float [n]
  (cond 
    (number? n) (float n)
    (string? n) (try (Double/parseDouble n) (catch Exception _ n))
    :else n
  )
)
...
;                                                   ^^^ ^
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defn cargar-en-ult-reg [regs-de-act dir tipo valor]
  (cargar-en-reg-dest regs-de-act [(dec (count regs-de-act)) dir] tipo valor)
)
...
;                                 ^^^ ^
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defn cargar-en-reg-dest [regs-de-act [x y] tipo valor]
  (update regs-de-act x #(assoc % y [tipo valor]))
)
